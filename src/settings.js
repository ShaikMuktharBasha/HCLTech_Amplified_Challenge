...
export class SettingsComponent {
  constructor() {
    ...
    this.geminiOption = document.getElementById('engine-gemini-option');
    this.keySection = document.getElementById('gemini-key-section');
    this.keyInput = document.getElementById('settings-gemini-key');
  ...
  initEvents() {
    ...
    this.mockOption.addEventListener('click', () => {
      ...
      aiEngine.updateSettings('mock', store.loadFromStorage().settings.geminiKey);
    });

    // Select Gemini engine
    this.geminiOption.addEventListener('click', () => {
      store.setEngine('gemini');
      const key = store.loadFromStorage().settings.geminiKey;
      aiEngine.updateSettings('gemini', key);
    });
    ...
    this.testKeyBtn.addEventListener('click', () => this.testGeminiConnection());
  ...
  async testGeminiConnection() {
    ...
    this.testStatusMsg.textContent = 'Testing connection, sending secure request to Google...';
    ...
    try {
      ...
      if (isValid) {
        ...
        this.testStatusMsg.textContent = 'Success! Gemini API connection established successfully.';
      } else {
        this.testStatusMsg.className = 'key-test-status error';
